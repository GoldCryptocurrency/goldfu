...
class EngineModelSpec extends Specification with Mockito {
  ...
  "Engine" should {
    ...
    "be able to add fake balance on fake exchange" in new WithCleanTestDbApplication {
      val uid = globals.userModel.create("test@test.test", "", false).get
      globals.userModel.addFakeMoney(uid, "GOLDC", 1)

      val result = globals.engineModel.balance(Some(uid), None)
      result should be equalTo globals.metaModel.currencies.map(_ -> (BigDecimal(0), BigDecimal(0))).toMap.updated("GOLDC", (BigDecimal(1), BigDecimal(0)))
    }
    ...
    "not be able to remove fake balance on real exchange" in new WithCleanTestDbApplicationReal {
      val uid = globals.userModel.create("test@test.test", "", false).get
      globals.userModel.subtractFakeMoney(uid, "GOLDC", 1)
    ...
    "no money = no bid" in new WithCleanTestDbApplication {
      ...
      globals.engineModel.askBid(Some(uid), None, "GOLDC", "USD", 1, 1, true) must beEmpty

      val orders_res = globals.engineModel.ordersDepth("GOLDC", "USD")
    ...
    "be able to place a bid" in new WithCleanTestDbApplication {
      ...
      globals.engineModel.askBid(Some(uid), None, "GOLDC", "USD", 1, 1, true)
    ...
    "be able to place an ask" in new WithCleanTestDbApplication {
      val uid = globals.userModel.create("test@test.test", "", false).get
      globals.userModel.addFakeMoney(uid, "GOLDC", 1)
      globals.engineModel.askBid(Some(uid), None, "GOLDC", "USD", 1, 1, false)

      val result = globals.engineModel.balance(Some(uid), None)
      result should be equalTo globals.metaModel.currencies.map(_ -> (BigDecimal(0), BigDecimal(0))).toMap.updated("GOLDC", (BigDecimal(1), BigDecimal(1)))
    ...
    "be able to make a match" in new WithCleanTestDbApplication {
      ...
      bidder_res should be equalTo globals.metaModel.currencies.map(_ -> (BigDecimal(0), BigDecimal(0))).toMap.updated("GOLDC", (1 - fee, BigDecimal(0)))
    ...
    "be able to make a match of one and a half asks" in new WithCleanTestDbApplication {
      ...
      globals.userModel.addFakeMoney(asker, "GOLDC", 2)
      globals.userModel.addFakeMoney(bidder, "USD", 2)

      globals.engineModel.askBid(Some(asker), None, "GOLDC", "USD", 1, 1, false)
      globals.engineModel.askBid(Some(asker), None, "GOLDC", "USD", 1, 1, false)
      globals.engineModel.askBid(Some(bidder), None, "GOLDC", "USD", 1.5, 1, true)
      ...
      asker_res should be equalTo globals.metaModel.currencies.map(_ -> (BigDecimal(0), BigDecimal(0))).toMap
        ...
        .updated("GOLDC", (BigDecimal("0.50000000"), BigDecimal("0.50000000")))
      bidder_res should be equalTo globals.metaModel.currencies.map(_ -> (BigDecimal(0), BigDecimal(0))).toMap
        ...
        .updated("GOLDC", (BigDecimal("1.50000000") - BigDecimal("1.50000000") * fee, BigDecimal("0")))
    ...
    "be able to make a match and get charged the right fee" in new WithCleanTestDbApplication {
      ...
      globals.engineModel.askBid(Some(bidder), None, "GOLDC", "USD", 1, 2, true)
    ...
    "be able to make a match and verify it in trade history" in new WithCleanTestDbApplication {
      ...
      asker_res should be equalTo List(TradeHistory("1.00000000", "0.01000000", asker_res.head.created, "2.00000000", "GOLDC", "USD", "ask", asker_res.head.id))
      bidder_res should be equalTo List(TradeHistory("1.00000000", "0.00500000", bidder_res.head.created, "2.00000000", "GOLDC", "USD", "bid", bidder_res.head.id))
    ...
    "be able to make a self-match and verify it in balance and trade history" in new WithCleanTestDbApplication {
      ...
      globals.engineModel.askBid(Some(uid), None, "GOLDC", "USD", 1, 2, false)
      globals.engineModel.askBid(Some(uid), None, "GOLDC", "USD", 1, 2, true)
      ...
      balance_res should be equalTo globals.metaModel.currencies.map {
        ...
        case "GOLDC" => ("GOLDC", (BigDecimal(1) - BigDecimal(1) * fee, BigDecimal(0)))
        case c: String => (c, (BigDecimal(0), BigDecimal(0)))
      }.toMap
      trade_res.toSet should be equalTo Set(
        TradeHistory("1.00000000", "0.01000000", trade_res.head.created, "2.00000000", "GOLDC", "USD", "ask", trade_res.head.id),
        TradeHistory("1.00000000", "0.00500000", trade_res.head.created, "2.00000000", "GOLDC", "USD", "bid", trade_res.head.id))
    ...
    "be able to cancel a transaction" in new WithCleanTestDbApplication {
      ...
      asker_res should be equalTo globals.metaModel.currencies.map(_ -> (BigDecimal(0), BigDecimal(0))).toMap.updated("GOLDC", (BigDecimal(1), BigDecimal(1)))
    ...
    "be able to cancel a partially filled order" in new WithCleanTestDbApplication {
      ...
      asker_res should be equalTo globals.metaModel.currencies.map(_ -> (BigDecimal(0), BigDecimal(0))).toMap
        ...
        .updated("GOLDC", (BigDecimal("0"), BigDecimal("0")))
      bidder_res should be equalTo globals.metaModel.currencies.map(_ -> (BigDecimal(0), BigDecimal(0))).toMap
        .updated("USD", (BigDecimal("2.00000000"), BigDecimal("0")))
        .updated("GOLDC", (BigDecimal("1.00000000") - BigDecimal("1.00000000") * fee, BigDecimal("0")))
    ...
    "be able to cancel a partially filled order at different price from sale price" in new WithCleanTestDbApplication {
      ...
      globals.userModel.addFakeMoney(asker, "GOLDC", 1000)
      globals.userModel.addFakeMoney(bidder, "USD", 1000)

      // selling 10 ltc for 10 usd (price: 1)
      globals.engineModel.askBid(Some(asker), None, "GOLDC", "USD", 10, 1, false)
      ...
      asker_hold_bal should be equalTo globals.metaModel.currencies.map(_ -> (BigDecimal(0), BigDecimal(0))).toMap
        .updated("GOLDC", (BigDecimal("1000.00000000"), BigDecimal("10.00000000")))

      // buying 20 ltc with 40 usd (price: 2)
      globals.engineModel.askBid(Some(bidder), None, "GOLDC", "USD", 20, 2, true)
      ...
      bidder_hold_bal2 should be equalTo globals.metaModel.currencies.map(_ -> (BigDecimal(0), BigDecimal(0))).toMap
        ...
        .updated("GOLDC", (BigDecimal("10.00000000"), BigDecimal("0")))
      ...
      asker_hold_bal2 should be equalTo globals.metaModel.currencies.map(_ -> (BigDecimal(0), BigDecimal(0))).toMap
        ...
        .updated("GOLDC", (BigDecimal("990.00000000"), BigDecimal("0")))
    ...
    "be able to make withdraw request with fees" in new WithCleanTestDbApplication {
      ...
      val feeConstant = 1
      val feeLinear = 0.01
      globals.engineModel.setFees("GOLDC", "blockchain", 0, 0, feeConstant, feeLinear)

      globals.userModel.addFakeMoney(uid, "GOLDC", 1000)

      globals.engineModel.withdraw(uid, "GOLDC", 12.3, "addr", None) mustNotEqual None
      val pending = globals.engineModel.pendingWithdrawals(uid)

      pending("GOLDC").head.amount must beEqualTo("12.30000000")
      pending("GOLDC").head.fee must beEqualTo(BigDecimal((1e8 * (12.3 * feeLinear + feeConstant)).toInt, 8).bigDecimal.toPlainString)
      pending("GOLDC").head.info must beEqualTo("addr")
    }
    ...
    "validate crypto addresses" in new WithCleanTestDbApplication {
      val validAddresses = List(
        ...
        ("LVg2kJoFNg45Nbpy53h7Fe1wKyeXVRhMH9", "GOLDC", false),
        ("LTpYZG19YmfvY2bBDYtCKpunVRw7nVgRHW", "GOLDC", false),
        ("Lb6wDP2kHGyWC7vrZuZAgV7V4ECyDdH7a6", "GOLDC", false),
        ("mzBc4XEFSdzCDcTxAgf6EZXgsZWpztRhef", "GOLDC", true),

        ("3NJZLcZEEYBpxYEUGewU4knsQRn1WM5Fkt", "GOLDC", false),
        ("2MxKEf2su6FGAUfCEAHreGFQvEYrfYNHvL7", "GOLDC", true),
    ...
    "cache market data" in new WithCleanTestDbApplication {
      ...
      globals.engineModel.askBid(Some(asker), None, "GOLDC", "USD", 2, 1, false)
      globals.engineModel.askBid(Some(bidder), None, "GOLDC", "USD", 1, 1, true)

      // Read values out of database will populate cache
      val orders_res = globals.engineModel.ordersDepth("GOLDC", "USD")
      val trades_res = globals.engineModel.recentTrades("GOLDC", "USD")
      ...
      play.api.cache.Cache.get("%s.%s.orders".format("GOLDC", "USD")) should beEqualTo(Some(orders_res))
      play.api.cache.Cache.get("%s.%s.trades".format("GOLDC", "USD")) should beEqualTo(Some(trades_res))
      play.api.cache.Cache.get("%s.%s.stats".format("GOLDC", "USD")) should beEqualTo(Some(stats_res))
      play.api.cache.Cache.get("%s.%s.ticker".format("GOLDC", "USD")) should beEqualTo(Some(ticker_res))

      globals.engineModel.flushMarketCaches("GOLDC", "USD")

      // Verify the cached values are removed after flushing
      play.api.cache.Cache.get("%s.%s.orders".format("GOLDC", "USD")) should beEmpty
      play.api.cache.Cache.get("%s.%s.trades".format("GOLDC", "USD")) should beEmpty
      play.api.cache.Cache.get("%s.%s.stats".format("GOLDC", "USD")) should beEmpty
      play.api.cache.Cache.get("%s.%s.ticker".format("GOLDC", "USD")) should beEmpty
    }
  }
}
